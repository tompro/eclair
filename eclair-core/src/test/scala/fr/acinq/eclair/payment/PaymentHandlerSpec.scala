...
class PaymentHandlerSpec extends TestKit(ActorSystem("test")) with fixture.FunSuiteLike {
  ...
  test("Payment request generation with trampoline support") { _ =>
    ...
    {
      val handler = TestActorRef[LocalPaymentHandler](LocalPaymentHandler.props(Alice.nodeParams.copy(enableTrampolineRouting = false)))
    ...
    {
      val handler = TestActorRef[LocalPaymentHandler](LocalPaymentHandler.props(Alice.nodeParams.copy(enableTrampolineRouting = true)))
      sender.send(handler, ReceivePayment(Some(42 msat), "1 coffee"))
      val pr = sender.expectMsgType[PaymentRequest]
      assert(!pr.features.allowMultiPart)
      assert(pr.features.allowTrampoline)
