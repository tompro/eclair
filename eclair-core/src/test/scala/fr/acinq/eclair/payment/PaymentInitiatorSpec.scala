...
import fr.acinq.eclair.payment.PaymentInitiator.{SendPaymentConfig, SendPaymentRequest, SendTrampolinePaymentRequest}
...
class PaymentInitiatorSpec extends TestKit(ActorSystem("test")) with fixture.FunSuiteLike {
  ...
  test("forward trampoline payment") { f =>
    import f._
    val features = Features(Features.PAYMENT_SECRET_OPTIONAL, Features.BASIC_MULTI_PART_PAYMENT_OPTIONAL, Features.TRAMPOLINE_PAYMENT_OPTIONAL)
