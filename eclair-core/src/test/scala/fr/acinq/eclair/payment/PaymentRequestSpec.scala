...
class PaymentRequestSpec extends FunSuite {
  ...
  test("trampoline") {
    ...
    val pr1 = PaymentRequest(Block.LivenetGenesisBlock.hash, Some(123 msat), ByteVector32.One, priv, "Some invoice", features = Some(Features(Features.TRAMPOLINE_PAYMENT_OPTIONAL)))
    assert(!pr1.features.allowMultiPart)
    assert(pr1.features.allowTrampoline)

    val pr2 = PaymentRequest(Block.LivenetGenesisBlock.hash, Some(123 msat), ByteVector32.One, priv, "Some invoice", features = Some(Features(Features.TRAMPOLINE_PAYMENT_MANDATORY, Features.BASIC_MULTI_PART_PAYMENT_OPTIONAL)))
